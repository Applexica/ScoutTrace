...
func CmdClaudeHook(ctx context.Context, g *Globals, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(g.Stderr, "claude-hook: subcommand required (post-tool-use|install|snippet)")
		return 64
	}
	switch args[0] {
	case "post-tool-use":
...
func claudeHookCommand(g *Globals, flush bool, destination string) string {
	bin := g.ScoutBinary
	if bin == "" {
		bin = "scouttrace"
	}
	...
	parts = append(parts, "claude-hook", "post-tool-use")
	if destination != "" {
...
func claudeHookSnippet(g *Globals, args []string) int {
	...
	cmd := claudeHookCommand(g, *flush, *dest)
...
func claudeHookInstall(g *Globals, args []string) int {
	...
	cmd := claudeHookCommand(g, *flush, *dest)
	if err := appendClaudePostToolUseHook(settingsPath, cmd); err != nil {
		fmt.Fprintln(g.Stderr, "claude-hook install:", err)
		return 1
	}
	fmt.Fprintf(g.Stdout, "Installed Claude Code PostToolUse hook in %s\n", settingsPath)
...
func appendClaudePostToolUseHook(path, command string) error {
	root := map[string]any{}
	...
	hooks["PostToolUse"] = append(arr, entry)
